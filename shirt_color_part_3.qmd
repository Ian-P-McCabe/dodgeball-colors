...
The code is collapsed below, but work as follows

1. Initialize an empty model of the problem
2. Generate 103 variables representing if a color is selected or not
3. State that the sum of selected colors must be 16
4. Create an array that is the sum of all selected color pairs 
5. Solve the model (where the magic happens) 
6. Log results test test
...
After 5 minutes of solving, we obtain the following palette:

```{python}
#| output: false 

#selected_colors = [6, 12, 16, 18, 19, 24, 25, 35, 36, 46, 47, 49, 53, 62, 72, 88]
...
Lets sort make a heatmap showing how similar/different the colors are and take a closer look:
...
#selected_colors = [12, 16, 17, 19, 37, 49, 50, 52, 53, 70, 72, 74, 88, 97, 98, 100]
```
...
display_palette(selected_colors=selected_colors, color_df=color_df, filepath="assets/max_minimum_palette.svg")
```
...
plot_distance_heatmap(selected_colors, distance_matrix, color_df, filepath="assets/max_minimum_heatmap.svg")
```
