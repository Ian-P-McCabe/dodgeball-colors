...
Pantone provides LAB (and other color) information for each color with a PMS code.
...
Continuing where we left off in part 2, lets find the subset of colors with the greatest pairwise distance. This is the same metrix used in the random, greedy, and convex hull approaches. 
...
solver.parameters.max_time_in_seconds = solve_time  # Optional time limit
...
Lets sort make a heatmap showing how close the colors are and take a closer look:
...
To solve this issue, instead of maximizing total diversity, what if we tried to maximize the minimum distance between any two points?
...
# Solve
solver = cp_model.CpSolver()
solver.parameters.max_time_in_seconds = solve_time # May need more time for this formulation
solver.parameters.log_search_progress = True
status = solver.Solve(model)
