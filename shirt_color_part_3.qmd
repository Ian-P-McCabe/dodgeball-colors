...
The dodgeball league provides 103 unique shirt colors to pick from. Eagle eyed readers will see that there are 110 colors lsited below, however I have removed a few duplicate colors from the pool.
...
Most colors in the shirt color palette above have a PMS code beneath them, this stands for Pantone Matching System. 
PMS is a standardized color system that assigns unique numbers and names to specific colors, enabling designers, printers, manufacturers, 
and crazy dodgeball players to achieve consistent color production and analysis. 

Once a color has been located in the Pantone database (via a subscription service  ) we can obtain the LAB color value. 

For color difference calculations, will use the [CIEDE2000 distance](https://en.wikipedia.org/wiki/Color_difference#CIEDE2000). Color differences are returned as a number of ∆ E units.
...
selected_colors = [3, 10, 12, 13, 18, 19, 20, 22, 35, 46, 47, 53, 70, 88, 100, 101]
```
